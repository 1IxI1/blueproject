...
export class JettonMinter implements Contract {
  ...
  static discoveryMessage(owner: Address, include_address: boolean) {
    return beginCell()
      .storeUint(Op.minter.provide_wallet_address, 32)
      .storeUint(0, 64) // op, queryId
      .storeAddress(owner)
  ...
  static changeAdminMessage(newOwner: Address) {
    return beginCell()
      .storeUint(Op.minter.change_admin, 32)
      .storeUint(0, 64) // op, queryId
  ...
  static changeContentMessage(content: Cell) {
    return beginCell()
      .storeUint(Op.minter.change_content, 32)
      .storeUint(0, 64) // op, queryId
      .storeRef(content)
      .endCell();
  }
  ...
  static createSimpleMsgVotingMessage(
    ...
    let proposal = Voting.createSendMsgProposalBody(
  ...
  async sendCreateSimpleMsgVoting(
    provider: ContractProvider,
    via: Sender,
    expiration_date: bigint,
    minimal_execution_amount: bigint,
    ...
    await provider.internal(via, {
      sendMode: SendMode.PAY_GAS_SEPARATELY,
      body: JettonMinter.createSimpleMsgVotingMessage(
        expiration_date,
        minimal_execution_amount,
        payload,
        description,
  ...
  static createPollVotingMessage(
  ...
  static createCodeUpgradeMessage(
    minter_code: Cell | null,
    voting_code: Cell | null,
    query_id: bigint = 0n,
  ) {
    return beginCell()
      .storeUint(Op.minter.upgrade_code, 32)
  ...
  async getVotingAddress(
    provider: ContractProvider,
    voting_id: bigint,
  ): Promise<Address> {
    const res = await provider.get("get_voting_address", [
