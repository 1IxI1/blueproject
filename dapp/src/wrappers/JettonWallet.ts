...
export class JettonWallet implements Contract {
  ...
  static voteMessage(
    ...
    vote: boolean,
    need_confirmation: boolean = false,
  ) {
    return beginCell()
      .storeUint(Op.vote, 32)
      .storeUint(0, 64) // op, queryId
  ...
  static createSimpleMsgVotingMessageThroughWallet(
    expiration_date: bigint,
    ...
    return beginCell()
      .storeUint(Op.create_voting_through_wallet, 32)
      .storeUint(query_id, 64)
      .storeUint(voting_type, 64)
      .storeUint(expiration_date, 48)
      .storeRef(
        Voting.createSendMsgProposalBody(
  ...
  async sendCreateSimpleMsgVotingThroughWallet(
    provider: ContractProvider,
    via: Sender,
    expiration_date: bigint,
    minimal_execution: bigint,
    proposal: Cell,
    voting_type: bigint = 0n,
    ...
    await provider.internal(via, {
      sendMode: SendMode.PAY_GAS_SEPARATELY,
      value,
      body: JettonWallet.createSimpleMsgVotingMessageThroughWallet(
        expiration_date,
        minimal_execution,
        proposal,
        voting_type,
  ...
  async getVotedWeight(
    ...
    let res = await provider.get("get_voted_weight", [
      { type: "int", value: voting_id },
      { type: "int", value: expiration_date },
    ]);
    return res.stack.readBigNumber();
  }
  async getVoteKeeperAddress(
    provider: ContractProvider,
    voting_address: Address,
  ): Promise<Address> {
    const res = await provider.get("get_vote_keeper_address", [
      {
        type: "slice",
        cell: beginCell().storeAddress(voting_address).endCell(),
      },
    ]);
