...
export class Voting implements Contract {
  ...
  static votingConfigToCell(conf: VotingConfig) {
    return beginCell()
      .storeBit(false)
      .storeAddress(conf.master)
      .storeUint(conf.voting_id, 64)
      .endCell();
  }
  ...
  static endVotingMessage(query_id: bigint = 0n) {
    return beginCell()
      .storeUint(Op.voting.end_voting, 32)
...
*/
  ...
  static createPollProposal(
    voting_duration: bigint | number,
    body: Cell | string = "Sample description",
  ) {
