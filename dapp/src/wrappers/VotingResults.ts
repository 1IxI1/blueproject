...
export class VotingResults implements Contract {
    ...
    static createVoteResult(
        ...
        return beginCell()
            .storeUint(Op.minter.send_vote_result, 32)
    ...
    async sendProvideVoteResult(
        ...
        await provider.internal(via, {
            value,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell().storeUint(Op.results.provide_voting_results, 32).storeUint(query_id, 64).endCell(),
        });
    }
...
*/
    async getData(provider: ContractProvider) {
        ...
        return { init, votingBody, votingDuration, daoAddress, finished, votingId, votesFor, votesAgainst };
    }
    /*
...
*/
    static parseProvidedVoteResult(msgBody: Cell): VotingResultsData {
        ...
        if (op != Op.results.take_voting_results) throw new Error(`Invalid op: ${op}`);
