...
describe('DAO integrational', () => {
    ...
    let testKeeperContract: (
        wallet: ActiveJettonWallet,
        voting_addr: Address
    ) => Promise<SandboxContract<VoteKeeperTests>>;
    ...
    beforeAll(async () => {
        ...
        userWallet = async (address: Address) =>
            blockchain.openContract(JettonWallet.createFromAddress(await DAO.getWalletAddress(address)));
        votingContract = async (voting_id: bigint) =>
            blockchain.openContract(Voting.createFromAddress(await DAO.getVotingAddress(voting_id)));
        resultsContract = async (duration: number, votingBody: Cell) =>
        ...
        testVotingContract = async (voting_id: bigint) =>
            blockchain.openContract(VotingTests.createFromAddress(await DAO.getVotingAddress(voting_id)));
        voteKeeperContract = async (jw: ActiveJettonWallet, voting_addr: Address) =>
            blockchain.openContract(VoteKeeper.createFromAddress(await jw.getVoteKeeperAddress(voting_addr)));
        testKeeperContract = async (jw: ActiveJettonWallet, voting_addr: Address) =>
            blockchain.openContract(VoteKeeperTests.createFromAddress(await jw.getVoteKeeperAddress(voting_addr)));
        ...
        pickWinner = async (u1: ActiveWallet, u2: ActiveWallet) => {
            ...
            const mintAmount = comp.isEq || comp.hasZero ? getRandomTon(1, 10) : 0n;
            /*
            ...
            if (comp.isEq) {
                ...
                await DAO.sendMint(user1.getSender(), winner.address, mintAmount, toNano('0.05'), toNano('1'));
    ...
    it('should create new voting', async () => {
        ...
        let createSimpleMsgVoting = await DAO.sendCreateSimpleMsgVoting(
            user1.getSender(),
            expirationDate,
    ...
    it('jetton owner can vote for', async () => {
        ...
        /*
            ...
            */
        const res = await assertVoteChain(
            user1,
            user1JettonWallet,
            0n,
            initialUser1Balance,
    ...
    it('jetton owner can not transfer just after voting', async () => {
        const user1JettonWallet = await userWallet(user1.address);
        let transferResult = await user1JettonWallet.sendTransfer(
            user1.getSender(),
            toNano('0.1'), //tons
            1n,
            user1.address,
    ...
    it('jetton owner can transfer tokens which did not vote', async () => {
        ...
        await user2JettonWallet.sendTransfer(
            user2.getSender(),
            toNano('0.15'), //tons
            transferVal,
            user1.address,
    ...
    it('jetton owner can vote second time but only with new jettons', async () => {
        ...
        const voteReult = await assertVoteChain(
            user1,
            user1JettonWallet,
            walletData.locked,
            walletData.balance,
    ...
    it('jetton owner can vote in the other voting', async () => {
        ...
        const createSimpleMsgVoting = await DAO.sendCreateSimpleMsgVoting(
            user1.getSender(),
            expirationDate,
        ...
        let voteResult = await assertVoteChain(
            user1,
            user1JettonWallet,
    ...
    it('jetton owner can vote with confirmation', async () => {
        expirationDate = renewExp(expirationDate);
        await DAO.sendCreateSimpleMsgVoting(
            user1.getSender(),
            expirationDate,
            toNano('0.1'), // minimal_execution_amount
            beginCell().endCell() // payload
    ...
    it("jetton owner can't vote after expiration date", async () => {
        ...
        let res = await user1JettonWallet.sendVote(
            user1.getSender(),
            voting.address,
            expirationDate,
            voteFor,
        ...
        const sendVoteBody = VotingTests.submitVotesMessage(
            user1.address,
            expirationDate,
            userBalance,
            voteFor,
        ...
        res = await user2JettonWallet.sendVote(user2.getSender(), voting.address, expirationDate, voteFor, voteConfirm);
        ...
        expect(res.transactions).toHaveTransaction({
            ...
            body: JettonWallet.voteMessage(voting.address, expirationDate, voteFor, voteConfirm),
            success: false,
    ...
    it('Regular vote won', async () => {
        ...
        const votingRes = await DAO.sendCreateSimpleMsgVoting(
            user1.getSender(),
            expirationDate,
            toNano('0.1'), // minimal_execution_amount
            winMsg // payload
        ...
        expect(votingRes.transactions).toHaveTransaction({
            // init voting
            from: DAO.address,
            to: voting.address,
            success: true,
        ...
        await comp.winner.jetton.sendVote(comp.winner.user.getSender(), voting.address, expirationDate, true, false);

        await comp.loser.jetton.sendVote(comp.loser.user.getSender(), voting.address, expirationDate, false, false);
        ...
        expect(res.transactions).toHaveTransaction({
            from: voting.address,
            on: DAO.address,
            body: JettonMinterTests.createExecuteVotingMessage(
                votingId,
                expirationDate,
                defaultVotingType,
                voteData.votedFor,
    ...
    it('Vote lost', async () => {
        ...
        await comp.winner.jetton.sendVote(comp.winner.user.getSender(), voting.address, expirationDate, false, false);

        await comp.loser.jetton.sendVote(comp.loser.user.getSender(), voting.address, expirationDate, true, false);
    ...
    it('Voting should only accept votes from keeper', async () => {
        ...
        let res = await voting.sendSubmitVote(user1.getSender(), user1.address, expirationDate, voteNum, true, false);
        expect(res.transactions).toHaveTransaction({
            from: user1.address,
            on: voting.address,
            success: false,
        ...
        res = await voting.sendSubmitVote(
            blockchain.sender(keepR.address),
            user1.address,
            expirationDate,
            voteNum,
            true,
    ...
    it('Voting should only submit votes before expiration date', async () => {
        ...
        let res = await voting.sendSubmitVote(
            blockchain.sender(keepR.address),
            user1.address,
            expirationDate,
            voteNum,
    ...
    it('Keeper should only accept messages from corresponding jetton wallet', async () => {
        ...
        const execAmount = getRandomTon(1, 10);
        let res = await DAO.sendCreateSimpleMsgVoting(
            user1.getSender(),
            expirationDate,
            execAmount, // minimal_execution_amount
            winMsg // payload
        ...
        const voteBody = VoteKeeperTests.requestVoteMessage(user1.address, expirationDate, balance, true, false);
        ...
        res = await keepR.sendRequestVote(
            blockchain.sender(otherJetton.address),
            user1.address,
            expirationDate,
            balance,
            true,
        ...
        res = await keepR.sendRequestVote(
            blockchain.sender(user1.address),
            user1.address,
            expirationDate,
            balance,
            true,
    ...
    it('Keeper should accept only new votes', async () => {
        ...
        res = await keepR.sendRequestVote(
            blockchain.sender(user1JettonWallet.address),
            user1.address,
            expirationDate,
            balance - 1n,
            true,
    ...
    it('Execute vote result should only allow voting address', async () => {
        ...
        let res = await testDAO.sendExecuteVotingMessage(
            user1.getSender(),
            votingId,
            expirationDate,
            defaultVotingType,
        ...
        res = await testDAO.sendExecuteVotingMessage(
            votingSender,
            votingId,
            expirationDate,
            defaultVotingType,
    ...
    it('DAO self-admin case', async () => {
        ...
        // Create voting
        let createVoting = await DAO.sendCreateSimpleMsgVoting(
            user1.getSender(),
            expirationDate,
            toNano('0.1'), // minimal_execution_amount
            chgMsg // payload
        ...
        await user2JettonWallet.sendVote(user2.getSender(), voting.address, expirationDate, true, false);
    ...
    it('should not create if not enough money for deploy voting', async () => {
        ...
        const successCreateRes = await DAO.sendCreateSimpleMsgVoting(
            user1.getSender(),
            expirationDate,
            execAmount, // minimal_execution_amount
            winMsg // payload
        ...
        const createFailRes = await DAO.sendCreateSimpleMsgVoting(
            user1.getSender(),
            expirationDate,
            execAmount, // minimal_execution_amount
            winMsg, // payload
        ...
        const createRes = await DAO.sendCreateSimpleMsgVoting(
            user1.getSender(),
            expirationDate,
            execAmount, // minimal_execution_amount
            winMsg, // payload
    ...
    describe('Poll type voting (with send result)', () => {
        ...
        it('should vote and end with sending', async () => {
            ...
            await comp.winner.jetton.sendVote(
                comp.winner.user.getSender(),
                voting.address,
                expirationDate,
                true,
                false
            ...
            expect(res.transactions).toHaveTransaction({
                ...
                success: true,
                body: VotingResults.createVoteResult(votingId, voteData.votedFor, voteData.votedAgainst),
            });
        ...
        it('VotingResults should not be inited not from the DAO', async () => {
            ...
            votingResults = await resultsContract(duration, pollBody);
            initBody = beginCell()
                .storeUint(Op.results.init_voting_results, 32)
                .storeUint(0, 64)
                .storeUint(votingId, 64)
                .endCell();
        ...
        it('VotingResults should not receive the results before init', async () => {
            ...
            const sendVoteResRes = await blockchain.sendMessage(
                internal({
                    from: DAO.address,
                    to: votingResults.address,
                    value: toNano('0.1'),
        ...
        it("VotingResults should not receive the results for not it's voting", async () => {
            ...
            for (const sendVoteResBody of [sendVoteResIdLower, sendVoteResIdHigher]) {
                let sendVoteResRes = await blockchain.sendMessage(
                    internal({
                        from: DAO.address,
                        to: votingResults.address,
                        value: toNano('0.1'),
        ...
        it('should not provide results if not enough gas', async () => {
            const provideResult = await votingResults.sendProvideVoteResult(
                user1.getSender(),
                PROVIDE_RESULTS_GAS_CONSUMPTION + fwd_fee - 1n
            );
            expect(provideResult.transactions).toHaveTransaction({
                from: user1.address,
                to: votingResults.address,
        ...
        it('should provide results', async () => {
            ...
            const provideResult = await votingResults.sendProvideVoteResult(
                user1.getSender(),
                PROVIDE_RESULTS_GAS_CONSUMPTION + fwd_fee
            );
            let responseBody: Cell;
            expect(provideResult.transactions).toHaveTransaction({
                from: votingResults.address,
    ...
    it('should fail on trying to create voting with type 2', async () => {
        ...
        const createBody = beginCell()
            .storeUint(Op.minter.create_voting, 32)
            .storeUint(0, 64) // op, queryId
            .storeUint(expirationDate, 48)
            .storeRef(Cell.EMPTY) // proposal
    ...
    it('should not create type 1 voting if only polls', async () => {
        ...
        const payload = getRandomPayload();
        const winMsg = genMessage(user1.address, payload);
        const createVotingRes = await _DAO.sendCreateSimpleMsgVoting(
            user1.getSender(),
            expirationDate,
            toNano('0.1'), // minimal_execution_amount
            winMsg // payload
